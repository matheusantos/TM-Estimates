...
class Application_Model_FuncaoTransacao extends Zend_Db_Table_Abstract {
    ...
    public function inserir(array $request) {
        if ($request['funcao'] == 'EE') {
            if (($request['TipDados'] < 5 && $request['ArqRef'] <= 2) || (($request['TipDados'] >= 5 && $request['TipDados'] <= 15) && ($request['ArqRef'] < 2))) {
                $Complexidade = 1;
                $pfTrans = 3;
            } else if (($request['ArqRef'] > 2 && $request['TipDados'] < 5) || ($request['ArqRef'] == 2 && ($request['TipDados'] >= 5 && $request['TipDados'] <= 15)) || ($request['TipDados'] > 15 && $request['ArqRef'] < 2)) {
                $Complexidade = 2;
                $pfTrans = 4;
            } else if (($request['TipDados'] >= 2 && $request['ArqRef'] > 15) || (($request['TipDados'] >= 5 && $request['TipDados'] <= 15) && $request['ArqRef'] > 2)) {
                $Complexidade = 3;
                $pfTrans = 6;
            }
        } else if ($request['funcao'] == 'SE') {
            if (($request['ArqRef'] <= 3 && $request['TipDados'] < 6 ) || (($request['ArqRef'] < 2) && ($request['TipDados'] >= 6 && $request['TipDados'] <= 19))) {
                $Complexidade = 1;
                $pfTrans = 4;
            } else if (($request['TipDados'] < 6 && $request['ArqRef'] > 3) || (($request['ArqRef'] >= 2 && $request['ArqRef'] <= 3) && ($request['TipDados'] >= 6 && $request['TipDados'] <= 19)) || ($request['TipDados'] > 19 && $request['ArqRef'] > 3)) {
                $Complexidade = 2;
                $pfTrans = 5;
            } else if (($request['TipDados'] > 19 && $request['ArqRef'] >= 2) || (($request['TipDados'] >= 6 && $request['TipDados'] <= 19) && ($request['ArqRef'] > 3) )) {
                $Complexidade = 3;
                $pfTrans = 7;
            }
    ...
    public function pfTotal_select($id) {
        $dao = new Application_Model_DbTable_Funcaotransacao();
        $select = $dao->select()
